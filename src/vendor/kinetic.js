// Vendored, byte-for-byte unmodified from jui-chart/src/brush/canvas/base/kinetic.js
// (juijs-chart 2.6.12's own `util.canvas.base.kinetic`, `extend: null`). This is a standalone,
// self-contained physics utility with zero chart-domain logic - not "our" code and not something
// this port hand-reimplements, per PORT_STATUS.md's Phase E policy exception (generic utility
// code that happens to live in jui-chart's source tree gets imported as-is + typed via a sibling
// `kinetic.d.ts`, permanently - see that file's own doc comment and PORT_STATUS.md's kinetic.js
// entry for the full policy writeup). DO NOT EDIT this file - any change here is a fork of the
// original, defeating the point of vendoring it unmodified. If the upstream file changes, re-copy
// it here wholesale and re-verify src/composables/kinetic.ts's thin instantiation shim still
// matches its shape.
export default {

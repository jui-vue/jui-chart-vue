// Vendored from https://github.com/imranghory/treemap-squared (treemap-squarify.js, "Treemap
// Squared 0.5"), MIT license, Copyright (c) 2012 Imran Ghory - see this repo's README.md
// "Licence" section for the full license text (there is no separate LICENSE file upstream).
// Confirmed via `npm view treemap-squared` -> 404: this library was never published to npm, only
// ever distributed via GitHub, so it cannot be added as a normal `package.json` dependency (same
// situation as this project's `src/vendor/kinetic.js`, which also lives in `jui-chart`'s own
// source tree rather than the installable `juijs-graph` npm package) - vendoring the raw file is
// the only faithful way to depend on it.
//
// PORT_STATUS.md's Phase F audit confirmed jui-chart's own `chart.brush.treemap.calculator`/
// `chart.brush.treemap.container` (src/brush/treemap.js) is a restructured adaptation of exactly
// this library: identical function set (Container/normalize/treemapSingledimensional/
// treemapMultidimensional/squarify/improvesRatio/calculateRatio/flattenTreemap/
// sumMultidimensionalArray), identical Container methods (getCoordinates/cutArea/shortestEdge),
// and the same idiosyncratic "the pseudocode in the Bruls paper has the direction of the
// comparison wrong, this is the correct one" bugfix comment reproduced near-verbatim. Unlike
// `kinetic.js`, jui-chart's own copy was already reorganized into its own module system (not kept
// as one droppable file), so THIS file is sourced directly from the confirmed upstream
// `imranghory/treemap-squared` project instead of copied out of `jui-chart/src` - the most
// faithful available source for the real algorithm.
//
// Below this comment, the file is the real `treemap-squarify.js` source, byte-for-byte, with
// exactly ONE addition: the `export default Treemap;` statement at the very end. The upstream
// file predates ES modules (`var Treemap = {}` is meant as a plain browser global) and has no
// `export` of its own - that one line is the minimum necessary to consume it as an ES module from
// TypeScript/Vite, and is the ONLY line not present in the original. DO NOT otherwise edit this
// file - any change to the algorithm itself is a fork of the original, defeating the point of
// vendoring it. If the upstream file ever changes, re-copy it here wholesale (re-adding the same
// one-line export) and re-verify `src/composables/useTreemap.ts`'s thin shim still matches its
// shape.

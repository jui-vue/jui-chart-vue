...
describe('clustercolumn3d brush', () => {
  it('renders one extruded rect3d <g> per (row, target) cell inside a <g class="brush-clustercolumn3d">', () => {
    const wrapper = mount(Chart, {
      props: {
        ...
        brush: [{ type: 'clustercolumn3d', target: ['sales', 'profit'] }],
      },
    })

    const group = wrapper.element.querySelector('g.brush-clustercolumn3d')
  ...
  it('setup() defaults outerPadding:5, innerPadding:5', () => {
    const wrapper = mount(Chart, {
      props: {
        ...
        brush: [{ type: 'clustercolumn3d', target: ['total'] }],
      },
    })

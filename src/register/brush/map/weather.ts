// Port of legacy `src/brush/map/weather.js` ("chart.brush.map.weather", extend:
// "chart.brush.map.core") - draws a small "weather card" (rounded background + icon + title +
// info text) at each data row's `axis.map(id)` position. `IMAGES` (5 base64-encoded GIF icons,
// keyed by `weather` field value) is a byte-for-byte copy of the legacy file's own data URIs.
import { registerBrush, MapCoreBrush } from 'jui-graph-ts'
import type { BrushData } from 'jui-graph-ts'

type MapScaleFn = (id: string) => { x: number; y: number } | undefined

const W = 66
const H = 60
const R = 5

const IMAGES: Record<string, string> = {
  sunny:
    'data:image/gif;base64,',
  cloudy:
    'data:image/gif;base64,',
  murky:
    'data:image/gif;base64,',
  rain: 'data:image/gif;base64,',
  snow: 'data:image/gif;base64,',
}

function getFormat(brush: Record<string, unknown>, self: MapWeatherBrush, id: string): unknown {
  let name: unknown = null

  if (typeof brush.format === 'function') {
    name = self.format(id)
  }

  return name && name != '' ? name : id
}

export class MapWeatherBrush extends MapCoreBrush {
  private createWeather(id: string, name: unknown, uri: string, temp: unknown): any {
    const xy = ((this.axis as unknown as Record<string, unknown>).map as unknown as MapScaleFn)(id)
    if (xy == null) return undefined

    const g = this.chart.svg.group()
    const rect = this.chart.svg.rect({
      width: W + 2,
      height: H,
      rx: R,
      ry: R,
      fill: this.chart.theme('mapWeatherBackgroundColor'),
      stroke: this.chart.theme('mapWeatherBorderColor'),
      'stroke-width': 1,
    })
    const img = this.chart.svg.image({
      x: 1,
      y: 2,
      width: W,
      height: 45,
      'xmlns:xlink': 'http://www.w3.org/1999/xlink',
      'xlink:href': uri,
    })
    const title = this.chart.svg
      .text({
        x: W / 2,
        dy: -4,
        'font-size': this.chart.theme('mapWeatherFontSize'),
        'font-weight': 'bold',
        'text-anchor': 'middle',
        fill: this.chart.theme('mapWeatherTitleFontColor'),
      })
      .text(String(name))
    const info = this.chart.svg
      .text({
        x: W / 2,
        dy: 54,
        'font-size': this.chart.theme('mapWeatherFontSize'),
        'text-anchor': 'middle',
        fill: this.chart.theme('mapWeatherInfoFontColor'),
      })
      .text(String(temp))

    g.append(rect)
    g.append(img)
    g.append(title)
    g.append(info)
    g.translate(xy.x - W / 2, xy.y - H / 2)

    return g
  }

  draw = (): any => {
    const g = this.chart.svg.group()
    const brush = this.brush as Record<string, unknown>

    this.eachData((d) => {
      const row = d as BrushData
      const id = this.axis.getValue(row, 'id', null) as string
      const temp = this.axis.getValue(row, 'temperature', 0)
      const icon = this.axis.getValue(row, 'weather', 'sunny') as string

      g.append(this.createWeather(id, getFormat(brush, this, id), IMAGES[icon], temp))
    })

    return g
  }

  static setup(): Record<string, unknown> {
    return { format: null as ((...args: unknown[]) => unknown) | null }
  }
}

registerBrush('map.weather', MapWeatherBrush)

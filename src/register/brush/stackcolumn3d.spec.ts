...
describe('stackcolumn3d brush', () => {
  it('renders one stacked rect3d <g> per (row, target) cell (end-to-end along y) inside a <g class="brush-stackcolumn3d">', () => {
    const wrapper = mount(Chart, {
      props: {
        ...
        brush: [{ type: 'stackcolumn3d', target: ['sales', 'profit'] }],
      },
    })

    const group = wrapper.element.querySelector('g.brush-stackcolumn3d')
  ...
  it('setup() defaults outerPadding:10', () => {
    const wrapper = mount(Chart, {
      props: {
        ...
        brush: [{ type: 'stackcolumn3d', target: ['total'] }],
      },
    })

// Port of legacy `src/theme/dark.js` ("chart.theme.dark", extend: null) - a flat, plain
...
// values, cross-checked byte-for-byte against the real site's bundled `chart.js` dark theme
...
// an entire chart's render). Unlike `classic.ts` (which inlines its `colors` array directly in the returned
// object), `dark.js`/`gradient.js`/`pattern.js` each declare their own module-scope
// `themeColors` array first and reference it via `colors: themeColors` - a real structural
// difference in the legacy source, preserved as-is rather than inlined to match `classic.ts`'s
// own shape.
//

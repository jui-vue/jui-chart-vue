// Port of legacy `src/theme/gradient.js` ("chart.theme.gradient", extend: null) - a flat, plain
...
// values, cross-checked byte-for-byte against the real site's bundled `chart.js` gradient theme
...
// an entire chart's render). Like `dark.ts`, declares its own module-scope `themeColors` array (here, jui's
// own `"linear(top) #hex1,0.9 #hex2"` gradient-color mini-syntax strings, not plain hex colors -
// preserved literally; this project's own `colorUtil`/theme-resolution layer is what interprets
// that syntax downstream, unchanged by this port) and references it via `colors: themeColors`.
//

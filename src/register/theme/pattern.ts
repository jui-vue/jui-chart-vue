// Port of legacy `src/theme/pattern.js` ("chart.theme.pattern", extend: null) - a flat, plain
// style-value object with no logic at all, so this is a byte-for-byte transcription of that
// file's returned object (mechanically verified: every real style key matches the source exactly,
// key-for-key). The `map*` keys below were originally ported as dead COMMENTS (the `map.*` brush/
// widget family didn't exist in this project yet, so they were inert) - now UNCOMMENTED (live
// values, cross-checked byte-for-byte against the real site's bundled `chart.js` pattern theme
// block), since leaving them commented left every `map.*` demo using this theme with `undefined`
// style values for real SVG attributes (radius/color/etc.) - see `classic.ts`'s own header comment
// for the full Playwright-verified failure mode this caused (thrown errors, one of which aborted
// an entire chart's render). Like `dark.ts`, declares its own module-scope `themeColors` array (here, named
// SVG-pattern-fill identifiers - `"pattern-jennifer-01"` etc, referencing pattern definitions this
// project's own `src/pattern/*` family would supply - out of this task's own scope, same
// "downstream jui-chart extension, not a jui-graph-ts concern" category as
// `register/grid/topologytable.ts`) rather than literal colors - preserved literally, not resolved
// or validated against any pattern registry here.
//
...
registerTheme('pattern', patternTheme)
